...
export class AuthService {
  ...
  async login(user: any) {
    const payload = { username: user.email, sub: user._id, role: user.role, iss: process.env.JWT_ISSUER };
  ...
  async refreshToken(refreshToken: string) {
    try {
      ...
      const newAccessToken = this.jwtService.sign({ username: user.email, sub: user._id, role: user.role, iss: process.env.JWT_ISSUER });
